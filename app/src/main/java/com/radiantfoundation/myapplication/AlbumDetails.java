...
public class AlbumDetails extends AppCompatActivity {
    ...
    String albumName;
    ArrayList<MusicFiles> albumbSongs =new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i=0;i<musicFiles.size();i++)
        {
            ...
            {
                albumbSongs.add(j,musicFiles.get(i));
                j ++;

            }
        }
        byte[] image = getAbumArt(albumbSongs.get(0).getPath());
        if(image !=null)
        {
            Glide.with(this)
                    .load(image)
    ...
    protected void onResume() {
        super.onResume();
        if(!(albumbSongs.size()< 1))
        {
            albumDetailsAdapter= new AlbumDetailsAdapter(this,albumbSongs);
